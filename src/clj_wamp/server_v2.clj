...
(def project-version "clj-wamp/2.0.0")
...
(defn send-welcome!
  ...
  [sess-id]
  (send! sess-id (message-id :WELCOME) sess-id {}))

(defn send-abort!
  "Sends an ABORT message to abort opening a session."
  [sess-id details-dict reason-uri]
  (send! sess-id (message-id :ABORT) details-dict reason-uri))

(defn send-goodbye!
  "Send a GOODBYE message"
  [sess-id details-dict reason-uri]
  (send! sess-id (message-id :GOODBYE) details-dict reason-uri))
...
(defn send-call-result!
  ...
  (send! sess-id (message-id :CALL) call-id result))
...
(defn send-error!
  ...
  (send! sess-id (message-id :ERROR) call-id error-info error-uri))
